...
const GptSearchBar = () => {
  ...
  // Initialize GenAI and model once
  const genAI = new GoogleGenerativeAI(BARD_KEY);
  const model = genAI.getGenerativeModel({ model: "gemini-1.5-pro" }); // You can try "gemini-1.5-pro" too
  ...
  const handleGptSearchClick = async () => {
    try {
      ...
      const movies = text ? text.split(",").map((m) => m.trim()) : [];
